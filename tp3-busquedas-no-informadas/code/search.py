...
def depth_first_tree_search(problem, node_class):
    """
    [Figure 3.7]
    ...
    frontier = [node_class(problem.initial)]  # Stack
...
def depth_first_graph_search(problem, node_class):
    """
    [Figure 3.7]
    Search the deepest nodes in the search tree first.
    Search through the successors of a problem to find a goal.
...
def breadth_first_graph_search(problem, node_class):
    """[Figure 3.11]
    Note that this function can be implemented in a
    single line as below:
    return graph_search(problem, FIFOQueue())
    """
    node = node_class(problem.initial)
    if problem.goal_test(node.state):
        return node
    frontier = deque([node])
    explored = set()
...
def best_first_graph_search(problem, f, display=False, node_class=Node):
...
def uniform_cost_search(problem, display=False, node_class=Node):
    """[Figure 3.14]"""
    return best_first_graph_search(
        problem, lambda node: node.path_cost, display, node_class
    )


def depth_limited_search(problem, limit=50, node_class=Node):
    """[Figure 3.17]"""
    ...
    return recursive_dls(node_class(problem.initial), problem, limit)
